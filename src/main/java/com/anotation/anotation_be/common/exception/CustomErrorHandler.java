...
public class CustomErrorHandler {
    ...
    public ResponseEntity<?> handleBusinessException(final BusinessException e) {
        ...
        return new ResponseEntity<>(ApiResponse.fail(errorCode), HttpStatus.valueOf(errorCode.getStatus()));
    }

    @ExceptionHandler(IOException.class)
    public ResponseEntity<?> handleIOException(final IOException e) {
        log.warn(e.getMessage());
        return new ResponseEntity<>(ApiResponse.fail("C999", "입출력 과정에서 오류가 발생했습니다."), HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    public ResponseEntity<?> handleRuntimeException(final RuntimeException e) {
        log.warn(e.getMessage());
        return new ResponseEntity<>(ApiResponse.fail("C999", "이게 뭔 에러여 헤응"), HttpStatus.INTERNAL_SERVER_ERROR);
    }
}
