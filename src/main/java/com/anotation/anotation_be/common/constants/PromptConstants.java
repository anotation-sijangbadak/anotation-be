package com.anotation.anotation_be.common.constants;

public class PromptConstants {
    public static final String PROMPT = """
            - 당신은 사용자의 감정에 진심으로 공감하고, 그 감정이 담긴 음악을 선별하려고 노력합니다.
            - 동일한 감정 태그라도 문장 표현이 다르면 그 뉘앙스까지 고려하여 결과가 달라지도록 추천합니다.
            - 사용자는 이렇게 표현했습니다: "%s"
            - 당신은 사용자의 표현을 분석하여 다음과 같은 감정을 파악했습니다:
              %s
            - 감정은 중요도 순으로 나열되어 있으며, 1순위 감정에 가장 높은 가중치를 두고, 이후 순위에 따라 중요도를 낮추어 고려하세요.
            - 1순위 감정은 전체 추천 기준의 50퍼센트 이상 반영하고, 2순위 감정 30퍼센트, 3순위 감정 20퍼센트 비중으로 고려하세요.
            - 감정 스펙트럼 근접성에 따라 유사 감정 간의 교차 매칭을 허용합니다.
            - 사용자가 선호하는 음악 장르는 %s입니다.
            
            [지침]
            - 파악된 감정과 장르를 기준으로, 감정 해소나 증폭을 시도하지 말고 감정 자체와 밀접한 음악을 추천하세요.
            - 추천하는 곡은 최근 %d년 이내에 발매된 음악 중에서 멜론 연간 차트 Top10에 오를 정도의 너무 유명한 곡이 나오지 않게 선택하세요.
            - 추천 곡 수는 **15곡**으로 제한하세요.
            - %s
            
            [포맷]
            - 각 곡을 추천한 이유를 곡의 가사, 멜로디, 아티스트의 특성 등을 활용하여 `reason`에 반드시 명시하세요.
            - 결과는 반드시 **JSON 배열** 형태로, 각 항목은 `title`(노래 제목)과 `artist`(아티스트명), `reason`(선정 이유)만 포함하세요.
              예시: [{"title": "밤편지", "artist": "아이유", "reason": "아이유의 특유의 음색이 제시한 감정과 잘 부합함"}, {"title": "사건의 지평선", "artist": "윤하", "reason": "멜로디의 진행과 내부에 사용된 악기가 제시된 감정과 잘 부합함" }]
            - 추가 텍스트, 설명, 주석 없이 결과 배열만 출력하세요.
            - ``` 형태로 응답을 감싸지 마세요.
            - ** 같은 강조 표시를 하지 마세요.
            - "로 무조건 감싸고, 이스케이프 문자를 사용하지 마세요.
            
            [검증 조건]
            - **존재하지 않는 노래 제목이나 허구의 아티스트명**은 절대 생성하지 마세요.
            - **모르겠거나 확신이 없는 경우**, 절대 추천하지 말고 빈 배열([])을 반환하세요.
            - **감정 단어(예시: lonely, sadness 등)가 title에 그대로 들어가면 무효**입니다.
            - 당신이 존재 여부에 확신이 90퍼센트 이상일때만 추천하세요.
            - **곡명과 아티스트명을 변형하거나 번역하지 말고**, 원래의 공식 이름(한글 제목 포함)을 그대로 사용하세요.
            - **한국 정식 음원 서비스에서 확인 가능한 곡만 추천**하세요.
            """;

    /*
        temperature : 창의성(랜덤성) 정도
        top_p : 확률 누적 필터링 컷오프
        frequency_penalty : 같은 단어 반복 억제
        presence_penalty: 이전 단어 재사용 억제
        n : 생성할 답변 개수
    */
    public static final String PROMPT_CONFIG = """
            {
              "model": "gpt-4.1-mini",
              "messages": [
                          {
                            "role": "system",
                            "content": "당신은 감정에 민감한 음악 큐레이터입니다. 당신은 사용자의 감정 표현을 분석하고, 그 감정에 가장 밀접하게 부합하는 음악을 추천해야 합니다. 당신의 추천은 사실 기반이어야 하며, 존재하지 않는 곡이나 아티스트를 생성하지 않아야 합니다. 모르는 경우에는 답변을 생략하고, 거짓말을 하지 않습니다."
                          },
                          {
                            "role": "user",
                            "content": "%s"
                          }
                        ],
              "temperature": 0.3,
              "top_p": 0.85,
              "frequency_penalty": 0.2,
              "presence_penalty": 0.2,
              "n": 1
            }
            """;
}
